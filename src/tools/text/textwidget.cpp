  ...
  : QTextEdit(parent)
{
    ...
    setStyleSheet(QStringLiteral("TextWidget { background: #FF0000; }"));
...
void TextWidget::setTextColor(const QColor& c)
{
    QString s(
      ...
      QStringLiteral("TextWidget { background:  #FF0000); color: %1; }"));
    setStyleSheet(s.arg(c.name()));
}

// modification
void TextWidget::setTextColorAndBackground(const QColor& c, const QColor& bgColor)
{
    QString s(
      QStringLiteral("TextWidget { background: %1; color: %2; }"));
    setStyleSheet(s.arg(bgColor.name(), c.name()));
}
