...

{
    ...
    std::ifstream inputFile("/home/jenny/count.txt"); // Open file with full path

    if (inputFile.is_open()) { // Check if the file was successfully opened
        int countValue;
        inputFile >> countValue; // Read the count value from the file
        countValue = countValue + 1;
        m_context.circleCount = countValue; // Set the count value (assuming m_context is valid)
...
CaptureWidget::~CaptureWidget()
{
	...
	for (auto toolItem : m_captureToolObjects.captureToolObjects()) {
            ...
            if (toolItem->type() == CaptureTool::TYPE_CIRCLECOUNT) {
                ...
                std::ofstream outfile("/home/jenny/count.txt");
                outfile << toolItem->count() << std::endl; // Write to file
                outfile.close();
            }
	}
